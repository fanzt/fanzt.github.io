...
$$
\begin{aligned}
    \langle{0}|[Q,\Phi(x)]|{0}\rangle&=\int \mathrm{d}^{d-1}x' \langle{0}|[j^0(x'),\Phi(x)]|{0}\rangle \ , \\
    &=\int \mathrm{d}^{d-1}x' \sum\limits_{n}\int \frac{\mathrm{d}^{d-1}k}{(2\pi)^{d-1}}\left(\langle{0}|j^0(x')|{n_{\vec{k}}}\rangle\langle{n_{\vec{k}}}|\Phi(x)|{0}\rangle\right. \\
...
$$
\begin{aligned}
    \langle{0}|[Q,\Phi(x)]|{0}\rangle&=\int \mathrm{d}^{d-1}x' \sum\limits_{n}\int \frac{\mathrm{d}^{d-1}k}{(2\pi)^{d-1}} \, e^{-i k_{\mu} x'^{\mu}}\left(\langle{0}|j^0(0)|{n_{\vec{k}}}\rangle\langle{n_{\vec{k}}}|\Phi(x)|{0}\rangle\right. \\
    & \phantom{=\int \mathrm{d}^{d-1}x' \sum\limits_{n}\int \frac{\mathrm{d}^{d-1}k}{(2\pi)^{d-1}}}\left. -\langle{0}|\Phi(x)|{n_{-\vec{k}}}\rangle\langle{n_{-\vec{k}}}|j^0(0)|{0}\rangle \right)  \ ,\\
    &= \sum\limits_{n}\int \mathrm{d}^{d-1}k \, e^{-i E_n(\vec{k}) t^\prime}\,\delta^3(\vec{k})\left(\langle{0}|j^0(0)|{n_{\vec{k}}}\rangle\langle{n_{\vec{k}}}|\Phi(x)|{0}\rangle\right. \\
    & \phantom{=\int \mathrm{d}^{d-1}x' \sum\limits_{n}\int \frac{d^{d-1}k}{(2\pi)^{3}}}\left. -\langle{0}|\Phi(x)|{n_{-\vec{k}}}\rangle\langle{n_{-\vec{k}}}|j^0(0)|{0}\rangle \right)  \ ,\\
\end{aligned}
$$
...
接下来我们要对 NG 粒子的物理意义作一些讨论。回想我们对自发对称性破缺存在的基本要求是真空态在对称变换下不会回到原本的真空态。但我们可以看到，变化后的真空态理论上还会是系统的基态，因为我们的 Lagrangian 是具有对称性的，因而相互作用项以及有效势都是具有相应的对称性的，因此原本的对称变换不会改变体系的能量。这意味着我们在每个时空位置上都有着可以用对称变换标记的，至少多于一个的真空态，即**简并真空**。由于我们的对称变换是连续的，这些简并真空便都可以被一个连续的变换联系在一起，也就是说它们之间不会有能隙或势阱。于是，一个沿着简并真空方向的微小涨落变化到二阶项为止都不会提供势能的变化，因而这些涨落态都是无质量的，而当我们考虑在每个点作一个微小涨落之后（注意，我们破缺的是整体对称性，但在这里我们的涨落是局域的；整体的涨落需要贡献一个乘以整个时空体积的能量大小，而这是不可能的，因此才能保证粒子可以被“卡”在一个破缺对称性的真空态中；局域变换不会有这个问题，实际上这说明了没有一个“破缺局域对称性”的真空态可以“卡住”粒子，也就是局域对称性不得发生自发破缺，即 Elitzur 定理[^6]），它们正是我们所需要的 NG 粒子。一方面，这可以从谱分解的讨论中看出，我们在 K-L 谱表示的证明中所计算的那个量不等于零正说明了中间态 `$|{n_{\vec{k}}}\rangle$` 在由守恒荷（守恒流）激发出的态 `$\langle{0}|j^0(x)$` 上有非零的投影，这表示其正是由破缺的对称变化生成的真空扰动产生的态；另一方面这一点也可以从一些具体模型中看出：我们最常见的标量场引发 $\mathrm{U}(1)$ 对称性破缺总会在最终将标量场写为：
...
然而，这一结论并不完整，至少其不适用于时空对称性的自发破缺。这是因为，`$c_a(\vec{r})$` 是一个与未破缺平移对称性的剩余维度中的位矢 $r$ 相关的参数[^5]，这使得如果某些生成元之间产生了相互抵消，即下述方程如果有 $n$ 个非平凡解：
...
[^6]: S. Elitzur. Impossibility of spontaneously breaking local symmetries. Phys. Rev. D,12:3978–3982, Dec 1975.
