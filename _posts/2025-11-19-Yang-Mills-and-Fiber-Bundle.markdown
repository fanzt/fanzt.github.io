...
对于物理学关心的 Yang-Mills 理论，我们所需要的是一类特殊的纤维丛，其纤维流形 $F$ 被取为一个 Lie 群 $G$，结构群也被取为 Lie 群 $G$。这样的纤维丛称为**主丛**（Principal Bundle），简记为 $P(M,G)$；在本文中我们主要关注这类纤维丛。
...
容易识别出 $\omega$ 在全局右作用下按伴随表示变换，并且有性质：
...
从这一规范变换的定义（以及我们已经用到现在的符号）可以显然地看出，规范场 $A$ 对应于底流形上的联络 $A$，规范场强 $F$ 对应于底流形上的曲率 $F$。

## Associated Bundle and Matter Fields

TBD.

## Nontrivial Principal Bundle and Monopole
